...
1. The applicant has a history of bankruptcies. 
	...
	Neither of them is fatal; however, more information is needed in each case. At this 
	...
	application. However, if it happened more than 2 years ago, the system must employ 
	the **fuzzy logic** and decide whether to approve or not.  

2. The applicant has a history of making late payments. 

   1.  30-to-60-day late payments.
	If the applicant falls into this category then he/she needs to answer how many times 
	he/she has been late with this type of late payments. 
	If it is more than four times, then it is an automatic denial. 
	If he/she has been late four or fewer times, then the **fuzzy logic** needs to be 
   ...
   2.  60-to-90-day late payments 
	...
	If the applicant has acquired more than two records of this type of 
	negligence, then it is an automatic denial. 
	If the applicant has only one or two records of this type of
	negligence, then the **fuzzy logic** must be utilized. Also, 
	extenuating circumstances such as family issues must be 
	considered towards a more positive overall outlook. 

   3.   90-and-more-day late payments 
        If the applicant has acquired any of this type of late payments, 
	then it means an automatic denial. 
...
3. The applicant has collection accounts. 
	...
	1. Medical Collection Accounts.
		...
		Medical Collection Accounts usually appear due to often mistakes of insurance
		companies. Due to various mix-ups, sometimes, insurance companies end 
		up not paying their customers' medical bills. If the applicant know about their 
		existence and taking all necessary steps to resolve the issue, then it is not 
		a big deal, and usually does not introduce negative overall outlook. In such 
		cases, the fuzzy logic needs to be incorporated and the final decision to be 
		considered based on another information. 
		However, if the application does not know about their existence and is not 
		about to take any steps to resolve the issue, then it is an automatic denial. 

	2. Non-Medical, so-called "Irresponsible/Immature" collections accounts. 

		This case considered to be one of the worst possible cases. This sub-branch 
		basically means that the applicant is not willing to pay his monthly bills. 
		...
		an automatic denial. 
		If the applicant agrees to pay off one of his biggest collection accounts, then the 
		case gets passed to the loan officer; however, the chance of approval is very
		slim. 


##Mortgage Expert 

Seth Robinson, Senior Mortgage Loan Originator, Envoy Mortgage, my company seeks to provide the American dream of home ownership to all. My daily job is to call potential mortgage leads and convert them into sales. Also, maintain relationships with realtors and potential clients for future sales.
cell-phone: (404) 518-4587		     email: robinsoncet@gmail.com


##Simplifications and Constrains

1. Considering the size of this type of expert system in production, I had no choice but to constrain my application to do only one range out of many credit score ranges. However, I chose to implement the most difficult range which goes from 450 and up to 640 score points. The applicants who fall in this range, usually have to meet a number of additional requirements, which have to be taken into consideration and thoroughly analyzed. 
2. Due to the time constraints, I implemented only 3 sub-trees which lead to the mortgage approval decision and rate assignment. This includes: chapter 7 bankruptcy, chapter 13 bankruptcy and 30-60 day late payments with extenuating circumstances like 'family issues'. It might not seem a lot; however, my knowledge expert spent around 3 hours just on filling out the FAM rules for the project. 
3. The system does not make a decision whether to assign a 15-year mortgage or instead go with a 30-year offer. 
4. Considering that Credit Rating, LTV and DTI are used in each possible scenario, I decided to employ fuzzy logic and convert these three crisp input values into one overall score. This way I could decrease the number of FAM cases in each sub-tree by order of 3. For example, in chapter 7 case, instead of having an 81 case FAM, I achieved to have just 27 cases. However, I am not sure if doing so I did not lose some precision and sensitivity. 
...
1. To better understand the organization of the system, please refer to the flow_chart.png, which is located in the same folder with this report. During the video calls with my expert, I was making numerous notes and small flow-charts. After all information was collected, I turned everything into one flowchart on my computer. It turns out that nicely done flow charts - is on of the best tools for this type of projects. After finishing constructing the flowchart, I moved to coding and began converting everything to clips. 

2. The first task of the system is to collect the most necessary information concerning the applicant's case. This information includes: the applicant's credit score, recurring monthly debt, gross monthly income, market value of the house and the amount of the down payment. As soon as this information is provided, the system calculates the DTI and LTV ratios.
...
<img width="500" src="https://cloud.githubusercontent.com/assets/3220686/20918955/5ea76908-bb4e-11e6-9359-2c8c6f31e699.png">

After a set of additional case-related questions has been asked, the system calculates the eventual mortgage rate and decides whether to approve the applicant's application or not. To utilize the fuzzy logic, the system uses OVERALL OUTLOOK points and from one to n numbers of additional fuzzy sets, from a corresponding sub-tree. For example, to make a final decision in a chapter 7 case,  the system uses OVERALL OUTLOOK points (2.4) and the fuzzy set which summarizes how long ago the last bankruptcy happened (2.5). 
The process for calculating the bankruptcy chapter 13 case is very similar. It is also based 
OVERALL OUTLOOKS points (2.4) and a corresponding value for the time when the last bankruptcy happened (2.5). 


##1.3 The process of decision making for 30-60 day late payments with family extenuating circumstances. 
<img width="500" src="https://cloud.githubusercontent.com/assets/3220686/20918959/64d1ef74-bb4e-11e6-81ba-004c6cd40654.png">

In this case the fuzzy logic is employed to calculate the final decision and mortgage rate for the case when the applicant has made a 30-60 day late credit card payments, during the time of going through some family extenuating circumstances. The frequency fuzzy set stands for the number of late payments acquired by the applicant (2.7). To make the final decision the fuzzy logic is utilized using OVERALL OUTLOOK points (2.4) and frequency fuzzy set (2.7).
...
To make the final decision the system runs all of the corresponding values through the Rate Fuzzy Set (2.8) and  Approval Fuzzy Set (2.9). After this step, the system saves two crisp values for the rate and for the approval status. For the rate, the system does not need to convert the value into something else. However, for the approval status, the system converts the crisp output into one of three cases: Approved, Human Decision or Denial, as show in Approval Fuzzy Set (2.9). 
